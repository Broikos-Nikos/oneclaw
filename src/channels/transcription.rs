...
///
/// Groq validates the filename extension â€” `.oga` (Opus-in-Ogg) is not in
...
pub async fn transcribe_audio(
    ...
    let mime = mime_for_audio(extension).ok_or_else(|| {
        anyhow::anyhow!(
            "Unsupported audio format '.{extension}' â€” accepted: flac, mp3, mp4, mpeg, mpga, m4a, ogg, opus, wav, webm"
        )
    })?;
